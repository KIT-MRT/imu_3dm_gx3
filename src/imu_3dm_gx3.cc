...
int main(int argc, char** argv) {
    ...
    try {
        ...
        ROS_ERROR("%s: Failed to open port %s with error %s", name.c_str(), port.c_str(),
                  error.what());
        return -1;
    }
    ...
    int baud;
    nh_private.param("baud", baud, 115200);

    string frame_id;
    nh_private.param("frame_id", frame_id, string("imu"));

    double delay;
    nh_private.param("delay", delay, 0.0);
    ...
    sensor_msgs::Imu msg;
    ros::Publisher pub = nh_private.advertise<sensor_msgs::Imu>("imu", 100);
    while (nh_private.ok()) {
